...
public:
  ...
  /**
   * @brief Populate routing table by enabling interface one by one for every node
   *
   * Make sense for routing protocols that can handle multiple alternative paths
   */
  static void PopulateAllPossibleRoutingTables ();

  /**
   * @brief Populate n routing tables. First table is a normal routing tables, the remaining n-1 tables
   * are generated by randomly adjusting metric of every link in the system
   *
   * !!! Side effect when n > 1 !!! Link metrics will be altered to random values
   *
   * Make sense for Path Splicing-type routing protocols
   *
   * @param n number of routing tables to populate
   * 
   * @see Murtaza Motiwala, Megan Elmore, Nick Feamster, and Santosh Vempala. 2008. Path splicing.
   * In Proceedings of the ACM SIGCOMM 2008 conference on Data communication (SIGCOMM '08). ACM, New York, NY, USA, 27-38. DOI=10.1145/1402958.1402963 http://doi.acm.org/10.1145/1402958.1402963
   */
  static void PopulateRandomRoutingTables (uint32_t n);
